require([
    ...
    var successHandlers = {
        ...
        debug: function (result, $container) {
            ...
            if (Array.isArray(result)) {
                ...
                var lisHtml = result.map(function (err) {
                    return '<li class="mm-biller-result_debug-item"><strong>' + err.date + '</strong><p>' + err.msg + '</p></li>';
                }).join('');
        ...
        error: function (result, $container) {
            ...
            if (Array.isArray(result)) {
                ...
                var lisHtml = result.map(function (err) {
                    return '<li class="mm-biller-result_error-item"><strong>' + err.date + '</strong><p>' + err.msg + '</p></li>';
                }).join('');
        ...
        test: function (result, $container) {
            ...
            var lisHtml = result.map(function (test) {
                ...
                var supportLinkHtml = test.support_link ?
                    '<a target="_blank" href="' + test.support_link + '" class="mm-biller-icon__help-rounded"></a>' : '';
                ...
                return '<li class="mm-biller-result_test-item ' + test.result_code
        ...
        version: function (result, $container) {
            ...
            if (currentVersion === latestVersion) {
                resultHtml = '<strong class="mm-biller-version mm-biller-icon__thumbs-up">'
                    + $.mage.__('Great, you are using the latest version.')
                    + '</strong>';
            } else {

                var translatedResult = $.mage.__('There is a new version available <span>(%1)</span> see <button type="button" id="mm-biller-button_changelog">changelog</button>.')
                    .replace('%1', latestVersion);

                resultHtml = '<strong class="mm-biller-version mm-biller-icon__thumbs-down">'
                    + translatedResult
                    + '</strong>';
            }
        ...
        changelog: function (result, $container) {
            ...
            var lisHtml = Object.keys(result).map(function (key) {
                ...
                return '<li class="mm-biller-result_changelog-item"><b>'
                    + version + '</b><span class="mm-biller-divider">|</span><b>'
                    + date + '</b><div>'
                    + resultHtml + '</div></li>';
            }).join('');
    ...
    // init debug modal
    $(() => {
        initModal('#mm-biller-result_debug-modal', {
            type: 'popup',
            responsive: true,
            innerScroll: true,
            ...
            buttons: [
                {
                    ...
                    class: 'mm-biller-button__download mm-biller-icon__download-alt',
                    click: function () {

                        var elText = document.getElementById('mm-biller-result_debug').innerText || '';
        ...
        // init error modal
        initModal('#mm-biller-result_error-modal', {
            type: 'popup',
            responsive: true,
            innerScroll: true,
            ...
            buttons: [
                {
                    ...
                    click: function () {
                        ...
                        var elText = document.getElementById('mm-biller-result_error').innerText || '';
                        var link = document.createElement('a');
        ...
        // init selftest modal
        initModal('#mm-biller-result_test-modal', {
            type: 'popup',
            responsive: true,
            innerScroll: true,
        ...
        initModal('#mm-biller-result_changelog-modal', {
            type: 'popup',
            responsive: true,
            innerScroll: true,
            title: 'Changelog',
    ...
    $('.mm-biller-result_version-wrapper').loader({texts: ''});

    /**
     * Ajax request event
     */
    $(document).on('click', '[id^=mm-biller-button]', function () {
        var actionName = this.id.split('_')[1];
        var $modal = $('#mm-biller-result_' + actionName + '-modal');
        var $result = $('#mm-biller-result_' + actionName);

        if (actionName === 'version') {
            $(this).fadeOut(300).addClass('mm-biller-disabled');
            $modal = $('.mm-biller-result_' + actionName + '-wrapper');
            $modal.loader('show');
        } else {
        ...
        new Ajax.Request($modal.data('mm-biller-endpoind-url'), {
            loaderArea: false,
            asynchronous: true,
